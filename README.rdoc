finished:
	need to revert this to old commit (BUT BACK UP README FIRST!!!!) and re run without commas

	Patient scaffold:
	
	rails generate scaffold Patient user:references first_name last_name email qualified communication_preferred

	Doctor scaffold:
	rails generate scaffold Doctor user:references full_name, email, phone_number, fax_number, address, state, zipcode, mon, tue, wed, thu, fri, sat, sun
------------------------------
